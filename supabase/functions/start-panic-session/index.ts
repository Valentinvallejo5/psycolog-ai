...
serve(async (req) => {
  ...
  try {
    ...
    if (subscription?.plan === 'monthly' || subscription?.plan === 'annual') {
      ...
      await supabase.rpc('increment_panic_count', {
        p_user_id: user.id,
        p_date: today
      });
    ...
    const currentCount = usage?.panic_sessions_count || 0;
    ...
    if (!usage) {
      ...
      const { error: insertError } = await supabase
        ...
        .insert({
          user_id: user.id,
          date: today,
          panic_sessions_count: 1,
          meditation_sessions_count: 0
        });
      ...
      const { error: updateError } = await supabase
        .from('daily_usage')
        .update({ panic_sessions_count: currentCount + 1 })
        .eq('user_id', user.id)
        .eq('date', today);
